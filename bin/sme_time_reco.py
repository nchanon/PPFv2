...
if doKinDep==False:
    sme_file_2016 = TFile('results/2016/flattree/sme_matrices_alt_puinc.root')
    sme_file_2017 = TFile('results/2017/flattree/sme_matrices_alt_puinc.root')

    histSMEratio_2016 = []
    histSMEratio_2017 = []
    histSMEratio_2016_up = []
    histSMEratio_2017_up = []
    histSMEratio_2016_down = []
    histSMEratio_2017_down = []

    for ic in range(4):
	h_2016_tmp = TH1F('SME_model_2016_'+wilsonList[ic], 'SME_model_2016_'+wilsonList[ic], 24,0,24)
        h_2017_tmp = TH1F('SME_model_2017_'+wilsonList[ic], 'SME_model_2017_'+wilsonList[ic], 24,0,24)

        for it in range(24):
	    central_2016 = sme_file_2016.Get('signal_dilep_LO_amplitude_'+wilsonList[ic]+'_'+str(it)).GetMean()
	    err_2016 = sme_file_2016.Get('reco_2016_central_'+wilsonList[ic]+'_'+str(it)).GetMeanError()
	    central_2017 = sme_file_2017.Get('reco_2017_central_'+wilsonList[ic]+'_'+str(it)).GetMean()
	    err_2017 = sme_file_2017.Get('reco_2017_central_'+wilsonList[ic]+'_'+str(it)).GetMeanError()
	    h_2016_tmp.SetBinContent(1+it, central_2016)
	    h_2016_tmp.SetBinError(1+it, err_2016)
	    h_2017_tmp.SetBinContent(1+it, central_2017)
	    h_2017_tmp.SetBinError(1+it, err_2017)

	histSMEratio_2016.append(h_2016_tmp)
	histSMEratio_2017.append(h_2017_tmp)

    fOutput = TFile("/gridgroup/cms/nchanon/CMSSW_10_2_13/src/combine-ttbar/one_bin/inputs/SME_model.root","RECREATE")
    for h in histSMEratio_2016:
	fOutput.Write(h)
    for h in histSMEratio_2017:
	fOutput.Write(h)
    fOutput.Close()
